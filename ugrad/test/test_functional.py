...
def test_conv2d():
    ...
    f.backward(ugrad.TensorBase(grad_inp))
    ...
    tol = 1e-6
    # forward
    assert (np.abs(fut.data.numpy() - fpt.data.numpy()) < tol).all()
    # backward
    assert (np.abs(kut.grad.numpy() - kpt.grad.numpy()) < tol).all()
